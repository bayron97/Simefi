 ...
 */
@WebServlet("/recepcionProductosGeneralServlet")
@MultipartConfig
public class recepcionProductosGeneralServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public recepcionProductosGeneralServlet() {
		super();
				...
				for (int i = 0; i < noItem.length; i++) {
					...
					if(Integer.parseInt(cantidadEntrega[i])  > 0 && Integer.parseInt(estadocompra[i]) == 0) {
						...
						if(Integer.parseInt(cantidadEntrega[i]) != Integer.parseInt(cantidadentregadaplazo[i])) {
							...
							contratacionesDetalleGeneralDao.saveall(b);							
							}									
					}						
				}
			}
