 ...
 */
@WebServlet("/contratacionesDetalleServlet")
@MultipartConfig
public class contratacionesDetalleServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public contratacionesDetalleServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String[] tiemposEntrega = request.getParameterValues("tiemposEntrega");
...
//								b.setTiempoEntrega(tiemposEntrega[i]);
//								b.setCantidadComprado(Float.parseFloat(cantidadComprado[i]));
//								b.setCantidadEntregaPlazos(Integer.parseInt(cantidadEntregar[i].replaceAll(",", "")));
//								b.setFechaLimiteEntrega(fechaLimiteEntrega[i]);
//								
//								c=contratacionesDao.getByItem(Integer.parseInt(noItem[i]));
...
//								contratacionesDetalleDao.save(b);
//							}
//						}
//					}
				}
			...
			request.setAttribute("Id", statusG);
			...
			if (noItem != null) {
				for (int i = 0; i < noItem.length; i++) {	
					...
					if(IdDetalle[i].contentEquals("-1")) {
						...
						//contratacionesDetalleDao.update(b);
					}
