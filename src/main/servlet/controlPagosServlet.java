 ...
 */
@WebServlet("/controlPagosServlet")
@MultipartConfig
public class controlPagosServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public controlPagosServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		controlPagos a = new controlPagos();
		...
		if (Id.contentEquals("-1")) {
			...
			controlPagosDao.save(a);
			
		} else {
			System.out.print(step);
			if(step!=null){
				a = controlPagosDao.getById(Integer.parseInt(Id));
			...
			controlPagosDao.update(a);
		}

		request.setAttribute("throughServlet", "true");
		request.getRequestDispatcher("controlPagosALP.jsp").forward(request, response);
	}
