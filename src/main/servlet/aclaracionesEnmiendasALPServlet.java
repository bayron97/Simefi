 ...
 */
@WebServlet("/aclaracionesEnmiendasALPServlet")
@MultipartConfig
public class aclaracionesEnmiendasALPServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public aclaracionesEnmiendasALPServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//long correlativo = aclaracionEnmiendaALPDao.getNewCorrelative(tipo, noProcesoCompra);
		long correlativo = Long.parseLong(request.getParameter("cDocumentNumber"));
		...
		if (funcion != null && !funcion.isEmpty()) {
			switch (funcion) {
				case "nuevo":
					...
					int idAclaracionEnmienda = aclaracionEnmiendaALPDao.save(a);
					
					System.out.println("idAclaracionEnmienda "+ idAclaracionEnmienda);

					if (filePart != null && filePart.getSize() > 0) {
						a.setDocumento(tipo + "_" + noProcesoCompra + "_" + fecha +" - "+ idAclaracionEnmienda + ".pdf");
						InputStream fileContent = filePart.getInputStream();
						byte[] buffer = new byte[fileContent.available()];
						fileContent.read(buffer);

						String contextPath = getServletContext().getRealPath("/assets/files/amendmentsALPFiles/" + tipo
								+ "_" + noProcesoCompra + "_" + fecha +" - "+ idAclaracionEnmienda + ".pdf");
						...
						if (!dest.exists()) {
							...
							outStream.close();
							aclaracionEnmiendaALPDao.saveDocument(idAclaracionEnmienda, a.getDocumento());
					...
					if (wizardMode.equalsIgnoreCase("continuar")) {
						...
						session.setAttribute("funcion", "SINACCION");
						request.setAttribute("throughServlet", "true");
						request.getRequestDispatcher("amendmentsALP.jsp").forward(request, response);
					}
					break;
				case "modificar":
					aclaracionEnmiendaALP ma = new aclaracionEnmiendaALP();
					...
					if (mfilePart != null && mfilePart.getSize() > 0) {
						...
						outStream.write(buffer);
						outStream.close();
						aclaracionEnmiendaALPDao.updateWithDocument(Integer.parseInt(id), ma);
					} else {
						aclaracionEnmiendaALPDao.updateWithOutDocument(Integer.parseInt(id), ma);
					}
