 ...
 */
@WebServlet("/lugarAperturaMenorServlet")
public class lugarAperturaMenorServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public lugarAperturaMenorServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		lugarAperturaMenor a = new lugarAperturaMenor();
		a.setLugarApertura(lugarApert);
		a.setObservaciones(observacion);
		a.setEstado("INACTIVO");

		if (submit.contentEquals("nuevo")) {
			lugarAperturaMenorDao.save(a);
		} else {
			if (estado != null && !estado.isEmpty()) {
				a.setEstado("ACTIVO");
			}
			System.out.println(a.getEstado());
			lugarAperturaMenorDao.activar(a);
		}

		request.setAttribute("throughServlet", "true");
		request.getRequestDispatcher("paramMenorOpeningPlace.jsp").forward(request, response);
	}
