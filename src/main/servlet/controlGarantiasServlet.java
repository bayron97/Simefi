 ...
 */
@WebServlet("/controlGarantiasServlet")
@MultipartConfig
public class controlGarantiasServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public controlGarantiasServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String ordencompra = request.getParameter("ordencompraInsert");
		...
		String ValorOrdenCompra = request.getParameter("valorOrdenCompra");
		String moneda = request.getParameter("moneda");

		System.out.println(Id + "----------------------");
		
		controlGarantias a = new controlGarantias();
		...
		if (Id.contentEquals("-1")) {
			...
			if (filePart != null && filePart.getSize() > 0) {
				...
				if (!dest.exists()) {
					...
					a.setValorOrdenCompra(ValorOrdenCompra);
					a.setmoneda(moneda);
					
					controlGarantiasDao.save(a);
				}
			}
			...
			controlGarantiasDao.update(a);
		}

		request.setAttribute("throughServlet", "true");
		request.getRequestDispatcher("controlGarantiasALP.jsp").forward(request, response);
	}
